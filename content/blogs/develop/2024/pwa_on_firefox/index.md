...
- Windows:
  ...
  ![connector installation windows](./images/setup-1.webp)
- Linux
  ...
  ```
  - Debian/Ubuntu(DEB):
    ![connector installation deb](./images/setup-2.webp)
  - Redhat/Fedora/OpenSUSE(RPM):
    ![connector installation rpm](./images/setup-3.webp)

1. 接著會需要安裝不同於原生 Firefox 的 runtime，根據描述，會下載並由 connector 自動修改成支援 PWA 的 runtime。
   ![runtime installation](./images/setup-4.webp)
...
1. 首先需要打開你想要安裝成 Desktop App 的網頁，如果在不同的網頁嘗試設定，extension 會自動判斷該網頁 CORS Error，即便是 `https://www.notion.so` 與 `https://notion.so` 也屬於不同的 Origin。
   ...
   ![extension icon](./images/setup-5.webp)
1. 接著一步一步的跟著指引設定好該 PWA 的設定。
   ![add pwa](./images/setup-6.webp)
   ![add pwa](./images/setup-7.webp)
   ![add pwa](./images/setup-8.webp)
   ![add pwa](./images/setup-9.webp)
1. 完成，可以在應用程式選單內找到剛剛建立的 PWA 應用程式。
   ![pwa at menu](./images/setup-10.webp)
1. 開啟後可以自動開啟網頁應用程式，並具有獨立的設定檔。
   ![open pwa](./images/setup-11.webp)
