...
#   -w, --to-webp    Convert all PNG/JPG to WebP and delete originals
...
show_help() {
    ...
    echo "  -w, --to-webp     Convert all PNG/JPG to WebP and delete originals"
    ...
    echo "  $0 ./content -w                       # Convert all PNG/JPG to WebP"
    echo "  $0 ./content -w -d                    # Dry run WebP conversion"
}
...
convert_to_webp() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY-RUN] Would convert: $file (${width}x${height}, $size_human) -> $(basename "$webp_file")"
        return 0
    fi

    log_info "Converting: $file (${width}x${height}, $size_human)"

    if [[ $width -gt 1500 || $height -gt 1500 ]]; then
        magick "$file" -resize "1500x1500>" -quality 85 "$webp_file"
    else
        magick "$file" -quality 85 "$webp_file"
    fi
    ...
    if [[ -f "$webp_file" ]]; then
        ...
        log_success "Converted: $(basename "$file") -> $(basename "$webp_file") ($size_human -> $new_size_human, saved $saved_human)"
    else
...
if [[ "$TO_WEBP" == true ]]; then
    while IFS= read -r -d $'\0' file; do
        convert_to_webp "$file" || true
